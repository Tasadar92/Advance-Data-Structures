...
//Esta clase es necesaria para guardar el valor de la altura AVL en los nodos BTNodes
    ...
    public Position<E> first() {
    	Position<RBInfo<E>> pos = resBT.binTree.root();
    	
    	while(resBT.binTree.hasLeft(pos)){
    		pos = resBT.binTree.left(pos);
    	}
    ...
    public Position<E> last() {
    	...
    	while(resBT.binTree.hasRight(pos)){
    		pos = resBT.binTree.right(pos);
    	}
        return pos.getElement();
    }
	...
	public Iterable<Position<E>> successors(Position<E> pos) {
		...
		/*AVLInfo<E> aux = (AVLInfo) pos;
		Position<AVLInfo<E>> treePos = aux.getTreePosition();
		Position<AVLInfo<E>> p = bst.binTree.parent(treePos);*/
		
		nodeQueue.add(pos);
		l.add(pos);
		
		while(!nodeQueue.isEmpty()){
			RBInfo<E> aux = (RBInfo) nodeQueue.pollFirst();
			Position<RBInfo<E>> treePos = aux.getTreePosition();
			Position<RBInfo<E>> newPos = resBT.sucessor(treePos);
	...
	public Iterable<Position<E>> predecessors(Position<E> pos) {
		...
		Deque<Position<E>> nodeQueue = new LinkedList();
		ArrayList<Position<E>> l = new ArrayList<>();
		Position<RBInfo<E>> min = resBT.minimum(resBT.binTree.root());
		RBInfo<E> auxPos = (RBInfo) pos;
		Position<RBInfo<E>> treePos2 = auxPos.getTreePosition();
		...
		while(!nodeQueue.isEmpty()){
			...
			if(newPos != null && !newPos.equals(resBT.sucessor(treePos2))){
				l.add(newPos.getElement());
				nodeQueue.add(newPos.getElement());
